...
void SceneParser::parsePerspectiveCamera() {
    ...
    getToken(token);
    assert (!strcmp(token, "}"));
    camera = new PerspectiveCamera(center, direction, up, width, height, angle_radians, focus, radius);
}
...
Material *SceneParser::parseMaterial() {
    ...
    while (true) {
        ...
        if (strcmp(token, "color") == 0) {
            ...
            reflection = readFloat();
        } else if (strcmp(token, "refraction") == 0) {
            refraction = readFloat();
        } else if (strcmp(token, "refraction_rate") == 0) {
            refraction_rate = readFloat();
    ...
    auto *answer = new Material(color, light,  reflection, refraction, refraction_rate);
    return answer;
}
...
Plane *SceneParser::parsePlane() {
    ...
    Vector3f p2 = Vector3f::ZERO;
    Vector3f p3 = Vector3f::ZERO;
    Vector3f p4 = Vector3f::ZERO;
    while (true) {
        getToken(token);
        if (strcmp(token, "texture") == 0) {
            getToken(filename);
            texture = true;
        } else if (strcmp(token, "point1") == 0) {
            p1 = readVector3f();
        } else if (strcmp(token, "point2") == 0) {
            p2 = readVector3f();
        } else if (strcmp(token, "point3") == 0) {
            p3 = readVector3f();
        } else if (strcmp(token, "point4") == 0) {
            p4 = readVector3f();
        } else {
            assert (!strcmp(token, "}"));
            break;
    ...
    assert (current_material != nullptr);
    return new Plane(normal, offset, current_material, texture, filename, p1, p2, p3, p4);
}
